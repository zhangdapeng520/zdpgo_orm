...
// Config GORM config
type Config struct {
	// GORM perform single z01_add, update, delete operations in transactions by default to ensure database data integrity
	...
	QueryFields bool
	// CreateBatchSize default z01_add batch size
	CreateBatchSize int
...
// Session session config when z01_add session with Session() method
type Session struct {
	DryRun                   bool
	PrepareStmt              bool
	NewDB                    bool
...
// Session z01_add new db session
...
//
// db.ToSQL(func(tx *zdpgo_orm.DB) *zdpgo_orm.DB {
